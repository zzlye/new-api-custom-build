...
图片和视频提交后立即获得任务编号，由服务器继续生成。关闭页面不会中止已经接收的任务。

### 如何使用

1. 提交图片或视频生成请求，保存返回的任务编号。
2. 打开「任务日志」查看排队、生成、成功或失败状态，也可以使用任务编号查询结果。
3. 点击「查看生成结果」预览图片或播放视频。
4. 生成文件默认从任务完成起保留 **2 小时**。根用户可在「系统设置 → 运维 → 生成文件保存」中调整为 1 至 168 小时。
