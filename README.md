...
3. 点击「查看任务详情」，统一查看调用接口、模型、提示词、参考图（包括遮罩图）、生成图片或视频，以及提交、开始、响应、结束和到期时间。列表自动刷新时，已打开的详情会一直保留，直到主动关闭。
...
- 每份结果都可查看、复制本地预览地址；登录后可在新页面打开。服务方返回原始地址时，也会记录并提供复制入口。参考图的本地地址同样可查看。
- 仅返回图片数据的结果也有本地预览地址。地址随文件保存期限失效，上游原始地址的有效期由服务方决定。
