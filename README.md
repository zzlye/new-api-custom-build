...
3. 点击「查看任务详情」，统一查看调用接口、模型、提示词、参考图（包括遮罩图）、生成图片或视频，以及提交、开始、响应、结束和到期时间。任务列表和详情不自动刷新，需要时点击搜索或详情中的刷新按钮；已打开的详情一直保留，直到主动关闭。
4. 生成文件和上传的参考文件默认从任务完成起保留 **2 小时**。根用户可在「系统设置 → 运维 → 日志维护 → 生成文件保存」中调整为 1 至 168 小时。
5. 到期后生成文件自动清理，任务日志仍然保留。根用户可在任务结束后删除日志及对应文件，普通用户和管理员均没有删除权限。
6. 在「日志维护」中选择截止时间并确认「清理日志和任务」，可同时清理之前的使用日志、已结束的任务日志及对应参考图、图片和视频；运行中的任务会保留。手动清理会立即移除选中任务的文件，即使文件尚未到期。
7. 清理完成后分别显示使用日志和任务日志的删除数量。其他节点仍保存着文件的任务会保留并提示，文件清理失败也会保留对应记录供后续处理。

### 功能介绍

- 在「系统设置 → 安全与限制 → 速率限制」调整后台媒体并发数，默认 4，0 表示不额外限制，范围为 0 至 256，仅根用户可修改。速率限制控制一段时间内的请求次数；并发设置控制同时执行数，超过并发数才等待名额。
- 后台排队生成，页面关闭后仍会继续。
- 图片和视频只生成一次，失败后不自动重试或切换渠道重发；文件保存失败也直接记录失败，不自动重新下载。已有任务的进度查询和到期文件清理继续保留。
- 普通工具保持原来的返回方式；需要时可选择提交后立即获取任务编号。
- 图片与视频统一查看，支持按模型名称筛选；任务列表和详情由用户手动刷新，生成完成后的文件保存进度单独标明。
- 每份结果和参考图都可查看、复制本站文件地址，点击打开直接显示图片或播放视频，不再打开控制台预览页，也不展示服务方的原始地址。
- 仅返回图片数据的结果同样有本站文件地址。文件地址仅允许读取对应文件，并随保存期限失效。
- 任务详情跟随页面语言显示，中文页面中的字段和参数名称使用中文。
...
- 上游连接中断时会明确提示结果未确认及可能已计费；请先核对服务方的记录再决定是否重新生成，避免重复支出。
